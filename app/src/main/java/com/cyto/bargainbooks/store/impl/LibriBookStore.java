...
import com.cyto.bargainbooks.request.wishlist.impl.LibriWishlistRequest;
...
public class LibriBookStore implements BookStore {

    private BookFactory bf = new LibriBookFactory();
    ...
    public StringRequest getBookRequest(Book b, BookHandler bh, ErrorHandler eh) {
        return new LibriBookRequest(b, bh, eh).getStringRequest();
    }
    ...
    public boolean matchBookDetail(String url) {
        Pattern pattern = Pattern.compile("https:\\/\\/www\\.libri\\.hu\\/konyv\\/.+");
    ...
    public boolean matchWishlistUrl(String url) {
        Pattern pattern = Pattern.compile("https:\\/\\/www\\.libri\\.hu\\/kivansaglista\\/\\?.+");
        return pattern.matcher(url).find();
    }
    ...
    public WishlistRequest getWishListRequest(Context context, ListRequestHandler listHandler, @NotNull BookHandler bh, ErrorHandler eh) {
        return new LibriWishlistRequest(context, listHandler, bh, eh);
    }
}
